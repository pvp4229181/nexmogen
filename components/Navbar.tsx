...
export default function Navbar() {
  ...
  // An open menu pins the bar in place — tucking it away would take the panel with it.
  const menuLock = open || servicesOpen;
  useEffect(() => {
    if (menuLock) setTucked(false);
    lastY.current = window.scrollY;
    const onScroll = () => {
      const y = window.scrollY;
      const delta = y - lastY.current;
      setScrolled(y > 24);
      // Ignore sub-pixel jitter and overscroll bounce; only commit once past the height of the bar.
      if (!menuLock && Math.abs(delta) > 6 && y > 0) setTucked(delta > 0 && y > 160);
      lastY.current = y;
    };
    onScroll();
  ...
  return <header onFocus={() => setTucked(false)} className={`fixed inset-x-0 top-0 z-50 w-full max-w-full border-b transition-all duration-300 ${tucked ? "-translate-y-full" : "translate-y-0"} ${open ? "border-white/[0.08] bg-ink/85 shadow-lg shadow-black/30 backdrop-blur-xl" : "border-transparent bg-transparent backdrop-blur-none"}`}>
