...
const FloatingPanelsCanvas = dynamic(() => import("@/components/three/FloatingPanelsCanvas"), { ssr: false });
...
export default function Hero() {
  return <section className="relative min-h-[760px] w-full overflow-hidden bg-black sm:min-h-screen">
    <video className="absolute inset-0 h-full w-full object-cover opacity-70 grayscale contrast-125" src={HERO_VIDEO_URL} autoPlay loop muted playsInline preload="auto" aria-hidden="true" />
    ...
    <SceneGate className="pointer-events-none absolute -right-[18%] bottom-[-6%] z-[1] h-[86%] w-[88%] opacity-95 sm:-right-[8%] sm:w-[66%] lg:right-[-3%] lg:w-[55%]" fallback={<div className="absolute inset-[22%] rounded-full bg-[radial-gradient(circle,rgba(229,9,20,.3),transparent_68%)] blur-2xl" />}><FloatingPanelsCanvas /></SceneGate>
    <div className="pointer-events-none absolute inset-0 z-[2] bg-[linear-gradient(to_top,#050506_0%,transparent_26%)]" />
    <div className="container-px relative z-10 mx-auto flex min-h-[760px] max-w-7xl items-center py-28 sm:min-h-screen sm:py-32"><motion.div variants={container} initial="hidden" animate="show" className="min-w-0 max-w-4xl">
