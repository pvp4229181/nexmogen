...
export default function PageHero({ eyebrow, title, description, ctaLabel, ctaHref, icon }: { eyebrow: string; title: ReactNode; description?: string; ctaLabel?: string; ctaHref?: string; icon?: ReactNode }) {
  return (
    <section className="relative flex min-h-[70vh] w-full items-center overflow-hidden bg-black pt-20">
      <video className="absolute inset-0 h-full w-full object-cover opacity-55 grayscale contrast-125" src={HERO_VIDEO_URL} autoPlay loop muted playsInline preload="metadata" aria-hidden="true" />
      ...
      <div className="container-px relative z-10 mx-auto w-full max-w-7xl py-28 sm:py-36">
